...
else
{
    ...
    if ($action == 'create' || $action == 'edit')
    {
	 ...
	 jQuery("#addfamilleacte").click(function () {
                var t=jQuery("#listfamilleacte").children( ":selected" ).text();
                console.log("Add value t="+t)
                if (t != "" && t != " ")
                {
                    var box = jQuery("#familleacte");
                    u=box.val() + (box.val() != \'\' ? "\n" : \'\') + t;
                    box.val(u); box.html(u);
                    jQuery("#addfambox .ui-autocomplete-input").val("");
                    jQuery("#addfambox .ui-autocomplete-input").text("");
                    jQuery("#listfamilleacte").get(0).selectedIndex = 0;
                                        changed=true;
                        }
            });
...
/*        print '<br><b>'.$langs->trans("TypeVisite").'</b>: &nbsp; &nbsp; &nbsp; ';
	...
	print '<table class="notopnoleftnoright" id="addfambox" width="100%">';
        ...
        print '</td><td>';
	//print '<input type="text" size="3" class="flat" name="searchexamenprescrit" value="'.GETPOST("searchexamenprescrit").'" id="searchexamenprescrit">';
	listfamilleacte(1,400,'familleacte','');
        print ' <input type="button" class="button" id="addfamilleacte" name="addfamilleacte" value="+">';
        if ($user->admin) print ' '.info_admin($langs->trans("YouCanChangeValuesForThisListFromDictionarySetup"),1);
        print '</td></tr>';
        print '<tr><td class="tdtop">';
        print '</td><td>';
